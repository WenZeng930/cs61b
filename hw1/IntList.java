...
public class IntList {
    ...
    static IntList subTail(IntList L, int start) {
        ...
        if (start < 0 || L == null)
            ...
            if (start == 0) 
            // every IntList are constructed as IntList(A, IntList B), so we need to do it recursively
                return new IntList(L.head, subTail(L.tail, start));
            else
                return subTail(L.tail, start-1); // recursion
    }
    ...
    static IntList dsublist(IntList L, int start, int len) {
        ...
        if(start < 0 || L == null || len <= 0) 
            ...
            return dsublist(L.tail, start-1, len); // REPLACE WITH YOUR CODE */
